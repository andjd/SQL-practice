...
def all_teachers_join
  ...
  FROM
    teachers LEFT JOIN depts ON teachers.dept_id = depts.id
...
def all_depts_join
  ...
  SELECT
    teachers.name, depts.name
  FROM
    teachers RIGHT JOIN depts ON teachers.dept_id = depts.id
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    ...
    FROM
      depts LEFT JOIN teachers ON depts.id = teachers.dept_id
    GROUP BY
      depts.id
  SQL
end
